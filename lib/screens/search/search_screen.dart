...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _performSearch(PaperProvider paperProvider, String query) {
    ...
    if (query.isNotEmpty) {
      filtered = allPapers.where((paper) {
    ...
    // Apply dropdown filters (case-insensitive matching)
    filtered = filtered.where((paper) {
      // College filter
      if (_selectedCollege != null) {
        if (paper.collegeName.trim().toLowerCase() != 
            _selectedCollege!.trim().toLowerCase()) {
          return false;
        }
      }
      ...
      if (_selectedBranch != null) {
        if (paper.branch.trim().toLowerCase() != 
            _selectedBranch!.trim().toLowerCase()) {
          return false;
        }
      }
      ...
      if (_selectedExamType != null) {
        if (paper.examinationType.trim().toLowerCase() != 
            _selectedExamType!.trim().toLowerCase()) {
          return false;
        }
      }
  ...
  void _applyFilters(PaperProvider paperProvider) {
    ...
    _performSearch(paperProvider, _searchController.text);
  }

  bool _hasSearchOrFilters() {
                      ...
                      children: [
                        ...
                        TextButton.icon(
                          onPressed: () {
                            ...
                            // Set filtered papers to empty to show initial state
                            paperProvider.setFilteredPapers([]);
                          },
  ...
  Widget _buildSearchHint(IconData icon, String label, Color color) {
    return Column(
      children: [
        Container(
